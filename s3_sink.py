...
def dataframe(s3_uri):  #s3://ideakafkatest/QA_UI_testing/ftp/3/sftp/sftpuser1/demo-data/facts/INVENTORIES.csv
    try:
        
        df_list=[]
        
        bucket_name=s3_uri.split('/')[2]     #ideakafkatest
        s3_file_path=s3_uri.split(bucket_name)[1][1:]  #QA_UI_testing/ftp/3/sftp/sftpuser1/demo-data/facts/INVENTORIES.csv
        #print("--------",s3_file_path)
    ...
    try:
        ...
        with open("Output/report.txt",'a',newline='') as f:
            f.write(f"S3 Sink file Path : {s3_uri}\n")
            f.write(f"S3 Sink file Columns : {tuple(df1.columns)}\n")
            f.write(f"S3 Sink file Columns count : {df1.shape[1]}\n")
            f.write(f"S3 Sink file records count : {df1.shape[0]}\n\n")
...
def details(s3_uri):
    try:
        ...
        with open("Output/basic_details.txt",'a',newline='') as f:
            f.write(f"S3 Sink file Path : {s3_uri}\n")
            f.write(f"S3 Sink file Columns : {tuple(columns)}\n")
            f.write(f"S3 Sink file Columns count : {len(columns)}\n")
            f.write(f"S3 Sink file records count : {records}\n\n")
        ...
        with open('Output/errors.txt', 'a',newline='') as f:
            f.write(f"\n\n--- Exception {datetime.now().replace(microsecond=0)}---\n{e}")
            main_webm.status="[S3 Sink] Something Went Wrong !!"
